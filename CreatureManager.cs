...
public class Creature
{
    ...
    public class DropList
    {
        ...
        public Drop this[string prefabName] =>
            (drops ??= new Dictionary<string, Drop>()).TryGetValue(prefabName, out Drop drop)
                ? drop
    ...
    public Creature(string assetBundleFileName, string prefabName, string folderName = "assets") : this(
    ...
    public Creature(GameObject creature)
    {
        ...
        FoodItems = string.Join(",",
            creature.GetComponent<MonsterAI>()?.m_consumeItems.Where(i => i.m_itemData.m_dropPrefab)
                .Select(i => i.m_itemData.m_dropPrefab.name) ?? Enumerable.Empty<string>());
    }
    ...
    internal static void Patch_FejdStartup()
    {
        ...
        object? configManager = configManagerType == null
            ? null
        ...
        foreach (Creature creature in registeredCreatures)
        {
            ...
            tameConfigVisibility.Browsable = cfg.CanBeTamed.get() == Toggle.On;
            configWithDesc(cfg.ConsumesItemName, () => creature.FoodItems, updateAI, "Food items",
            ...
            spawnConfigVisibility.Browsable = cfg.Spawn.get() == SpawnOption.Custom;
            spawnConfig(cfg.SpecificSpawnTime, () => creature.SpecificSpawnTime, "Spawn time",
                "Configures the time of day for the creature to spawn.");
            spawnConfig(cfg.RequiredAltitude, () => creature.RequiredAltitude, "Required altitude",
                "Configures the altitude required for the creature to spawn.");
            spawnConfig(cfg.RequiredOceanDepth, () => creature.RequiredOceanDepth, "Required ocean depth",
                "Configures the ocean depth required for the creature to spawn.");
            spawnConfig(cfg.RequiredGlobalKey, () => creature.RequiredGlobalKey, "Required global key",
                "Configures the global key required for the creature to spawn.");
            spawnConfig(cfg.GroupSize, () => creature.GroupSize, "Group size",
                "Configures the size of the groups in which the creature spawns.");
            spawnConfig(cfg.Biome, () => creature.Biome, "Biome",
                "Configures the biome required for the creature to spawn.",
                new AcceptableEnumValues<Heightmap.Biome>(Enum.GetValues(typeof(Heightmap.Biome))
                    .Cast<Heightmap.Biome>().Where(e => e != Heightmap.Biome.BiomesMax).ToArray()));
            spawnConfig(cfg.SpecificSpawnArea, () => creature.SpecificSpawnArea, "Spawn area",
                "Configures if the creature spawns more towards the center or the edge of the biome.");
            spawnConfig(cfg.RequiredWeather, () => creature.RequiredWeather, "Required weather",
                "Configures the weather required for the creature to spawn.");
            spawnConfig(cfg.SpawnAltitude, () => creature.SpawnAltitude, "Spawn altitude",
                "Configures the height from the ground in which the creature will spawn.");
            spawnConfig(cfg.CanHaveStars, () => creature.CanHaveStars ? Toggle.On : Toggle.Off, "Can have stars",
                "If the creature can have stars.");
            spawnConfig(cfg.AttackImmediately, () => creature.AttackImmediately ? Toggle.On : Toggle.Off, "Hunt player",
                "Makes the creature immediately hunt down the player after it spawns.");
            spawnConfig(cfg.CheckSpawnInterval, () => creature.CheckSpawnInterval, "Maximum spawn interval",
                "Configures the timespan that Valheim has to make the creature spawn.");
            spawnConfig(cfg.SpawnChance, () => creature.SpawnChance, "Spawn chance",
                "Sets the chance for the creature to be spawned, every time Valheim checks the spawn.");
            spawnConfig(cfg.ForestSpawn, () => creature.ForestSpawn, "Forest condition",
    ...
    private void updateSpawnData(SpawnSystem.SpawnData spawnData)
    {
        ...
        spawnData.m_requiredGlobalKey =
            ((InternalName)typeof(GlobalKey).GetMember(cfg.RequiredGlobalKey.get().ToString())[0]
                .GetCustomAttributes(typeof(InternalName)).First()).internalName;
        spawnData.m_requiredEnvironments = Enum.GetValues(typeof(Weather)).Cast<Weather>()
            .Where(w => (w & cfg.RequiredWeather.get()) != Weather.None).Select(w =>
                ((InternalName)typeof(Weather).GetMember(w.ToString())[0].GetCustomAttributes(typeof(InternalName))
                    .First()).internalName).ToList();
    ...
    private static object? configSync
    {
        get
        {
            if (_configSync == null && hasConfigSync)
            {
                ...
                {
                    _configSync =
                        Activator.CreateInstance(configSyncType, plugin.Info.Metadata.GUID + " CreatureManager");
...
public static class PrefabManager
{
    static PrefabManager()
    {
        ...
        harmony.Patch(AccessTools.DeclaredMethod(typeof(ZNetScene), nameof(ZNetScene.Awake)),
            postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(Creature.DropList),
                nameof(Creature.DropList.AddDropsToCreature))));
        harmony.Patch(AccessTools.DeclaredMethod(typeof(SpawnSystem), nameof(SpawnSystem.Awake)),
            postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(Creature),
                nameof(Creature.AddToSpawnSystem))));
        harmony.Patch(AccessTools.DeclaredMethod(typeof(ObjectDB), nameof(ObjectDB.Awake)),
            postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(Creature),
                nameof(Creature.UpdateCreatureAis))));
        harmony.Patch(AccessTools.DeclaredMethod(typeof(FejdStartup), nameof(FejdStartup.Awake)),
            postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(Creature),
                nameof(Creature.Patch_FejdStartup))));
    }
